...
async def user_quota_limiter(request: ChatCompletionRequest, tracer, parent_span):
    try:
        ...
        with tracer.start_as_current_span(
            ...
            async with pool.acquire() as conn:
                user_id_quota_exists = await conn.fetchrow(
                    CHECK_IF_USER_QUOTA_LIMIT_EXISTS, user_id
                )
                ...
                if username:
                    metadata_user_quota_details[
                        "metadata.user_quota_details.username"
                    ] = username
        ...
        logging.error(
            "HTTP Exception. Status Code: %s Error: %s",
